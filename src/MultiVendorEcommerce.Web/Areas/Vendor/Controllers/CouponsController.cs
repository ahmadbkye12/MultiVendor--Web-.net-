...
public class CouponsController : Controller
{
    ...
    public async Task<IActionResult> Index(string? search = null, bool? isActive = null, int page = 1)
    {
        ...
        return View(await _mediator.Send(new GetMyCouponsQuery(search, isActive, page)));
    }

    public IActionResult Create() => View(new CouponFormViewModel { IsAdmin = false, IsActive = true });
    ...
    public async Task<IActionResult> Create(CouponFormViewModel vm)
    {
        ...
        if (me is null || !me.Stores.Any()) { TempData["Error"] = "You don't have a store."; return RedirectToAction(nameof(Index)); }

        var storeId = me.Stores.First().Id;
    ...
    {
        var c = await _mediator.Send(new GetCouponByIdQuery(id, IsAdmin: false));
        return View(new CouponFormViewModel
        {
            IsAdmin = false, Id = c.Id, Code = c.Code, DiscountType = c.DiscountType,
